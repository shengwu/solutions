...
def run_cmd(grid, cmd, minx, maxx, miny, maxy):
...
def run_brightness_cmd(grid, cmd, minx, maxx, miny, maxy):
    for x in xrange(minx, maxx+1):
        for y in xrange(miny, maxy+1):
            if cmd == 'turn_on':
                grid[x][y] += 1
...
def parse_row(row):
    ...
    if parts[0] == 'turn':
        if parts[1] == 'on':
            ...
            cmd = 'turn_off'
        minx, miny = split_coords(parts[2])
    else:
        cmd = 'toggle'
        minx, miny = split_coords(parts[1])
    maxx, maxy = split_coords(parts[-1])
    return (cmd, minx, maxx, miny, maxy)

grid = make_grid(1000)
brightness_grid = make_brightness_grid(1000)
for row in inp.strip().split('\n'):
    parsed = parse_row(row)
    run_cmd(grid, *parsed)
    run_brightness_cmd(brightness_grid, *parsed)

print sum(sum(row) for row in grid)
print sum(sum(row) for row in brightness_grid)
