...
def matches(orig, cand):
    for k in cand:
        if orig[k] != cand[k]: return False
    return True

orig = parse_sue('Sue 0: ' + ', '.join(data.strip().split('\n')))
rows = inp.strip().split('\n')
sues = []
for row in rows:
    sues.append(parse_sue(row))

for i, sue in enumerate(sues):
    if matches(orig, sue):
        print i+1

def matches2(orig, cand):
    for k in cand:
        if k in ('cats', 'trees'):
            if cand[k] <= orig[k]:
                return False
            ...
            if cand[k] >= orig[k]:
                return False
        elif orig[k] != cand[k]: return False
    return True

for i, sue in enumerate(sues):
    if matches2(orig, sue):
        print i+1
# i get two answers
# apparently 241 was correct
