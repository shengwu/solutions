...
Room = namedtuple('Room', ['common_letters', 'sector_id', 'parts'])

def parse_room(line):
    parts = line.split('[')
    common_letters = parts[1][:-1]
    first_parts = parts[0].split('-')
    sector_id = int(first_parts[-1])
    return Room(common_letters, sector_id, first_parts[:-1])

def most_common_five_letters(room_parts):
    '''
...
def get_id_if_real_room(room):
    if room.common_letters == most_common_five_letters(room.parts):
        return room.sector_id
    return 0

rooms = list(parse_room(line) for line in inp.strip().split('\n'))
print(sum(get_id_if_real_room(room) for room in rooms))

# part 2
# answer was 501

def shift_room(room):
    sent = ' '.join(room.parts)
    result = []
    for letter in sent:
        if letter == ' ':
            result.append(' ')
        else:
            shifted_n = ((ord(letter) - 97) + room.sector_id) % 26
            result.append(chr(shifted_n + 97))
    return ''.join(result)

for room in rooms:
    if shift_room(room) == 'northpole object storage':
