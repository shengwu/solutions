...
# very inefficient part 1
# answer was 47

cmds = set(inp.strip().split('\n'))
bots = defaultdict(set)
outputs = {}
for cmd in list(cmds):
    parts = cmd.split()
    if parts[0] == 'value':
        bots[int(parts[5])].add(int(parts[1]))
        cmds.remove(cmd)
while len(cmds) > 0:
    cmd = cmds.pop()
    parts = cmd.split()
    if parts[0] == 'bot':
        ...
        if len(bots[bot]) < 2:
            cmds.add(cmd)
            continue
        if bots[bot] == set([61, 17]):
            print bot
        low = min(bots[bot])
        hi = max(bots[bot])
        low_dest = int(parts[6]) 
        hi_dest = int(parts[11])
        bots[bot].remove(low)
        bots[bot].remove(hi)
        if parts[5] == 'bot':
            bots[low_dest].add(low)
        if parts[5] == 'output':
